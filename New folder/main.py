...
class AttendanceSystem:
    def __init__(self, root):
        ...
        self.serial_port = self.find_serial_port()  # Automatically find the port
        ...
        self.reply_to_email = "micovicencio55@gmail.com"  # Your reply-to email

    def find_serial_port(self):
        """
        Automatically finds an available serial port.
        This function iterates through available ports and tries to connect.
        You might need to refine this logic based on your specific device.
        """
        ...
        for p in ports:
            try:
                # Try opening the port with a short timeout to avoid hanging
                temp_ser = serial.Serial(p.device, self.baud_rate, timeout=1)
                temp_ser.close()  # Close it immediately if successful
                print(f"Found serial port: {p.device}")
                return p.device  # Return the port name if successful
            except (serial.SerialException, OSError):
                # Ignore ports that are already in use or don't exist
                pass
    ...
    def connect_serial(self):
        if self.serial_port:
            ...
            print("No serial port to connect to.")
